...
# wyrdness.github.io Repo Rules

## Generated outputs — never edit directly

These files are built outputs. Always regenerate via `make`; never edit by hand:
- `api/v1/index.json` — aggregated from all sibling phenomenon repos
...
All Node/npm tooling runs inside Docker via `Makefile`. Never run `node` or `npm` directly on the host.

Key targets:
- `make build` — full rebuild: aggregate-local + categories + stats + pages
- `make aggregate-local` — re-aggregate from sibling repos on disk
- `make validate-schema` — validate all sibling repo api.json files (runs ajv in Docker)
- `make repo-docs [ONLY={id}]` — regenerate README.md + SOURCES.md from api.json
- `make normalize-categories[-apply]` — dry-run or apply category normalization across all repos
- `make research ID={id} [APPLY=1] API_KEY=...` — research one phenomenon
- `make dev` — serve site at localhost:8000 (nginx in Docker)
- `make pages` — regenerate static pages only (faster than full build)

## Script placement rules

- `scripts/` — site-building only: `aggregate-api.js`, `aggregate-local.js`, `generate-categories.js`, `generate-index.js`, `generate-pages.js`, `generate-stats.js`
- `../.github/scripts/` — project-wide scripts invoked via this Makefile: validate, normalize, research, repo-docs
- Never add project-wide scripts to `scripts/`; never add site-building scripts to `.github/scripts/`
